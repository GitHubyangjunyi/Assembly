...
;   0x7C04 - 2 = 0x7C02
;结果大于实际使用的段界限加1,允许操作
;回到代码第67~69行,这三行加载栈段选择子,而栈段选择子的描述符设置栈的线性基地址为0x0000 7C00,段界限为0xF FFFE,粒度4KB,并设置esp=0
...
;!!!!!!!!!!!!!!!!!段界限理解成控制栈指针的变化范围,最大ESP没办法限制,那就限制最小ESP,可以结合第13章中内核建立用户程序栈段描述符那段代码!!!!
;我(杨俊艺)至于栈为什么要这么搞事情,就是如果我要定义一个栈段,并且已经确定好基地址从哪里开始,比如这里的0x0000 7C00作为基地址,大小也确定为4KB
;第一步:用基地址0x7C00加上段界限的最大值0xFFFF FFFF(因为ESP最大值也是0xFFFF FFFF)就得到这个栈的最高端线性地址0x7BFF
;第二步:用最高端线性地址0x7BFF-0x1000(栈大小)+1得到栈段最低端有效物理地址0x6C00,注意,0x6C00~0x7BFF之间刚好有4KB空间
;第三步:0x6C00-基地址0x7C00=实际使用的段界限+1
;第四步:解释为什么0x6C00-基地址0x7C00=实际使用的段界限+1,因为栈是向下扩展的,基地址0x7C00不可用,如果用0x6C00-基地址0x7C00得到的数如果作为偏移量传到ESP,那么ESP与基地址之和形成的物理地址就指向了0x7C00
;      所以实际使用的段界限(ESP允许的最小值)=0x6C00-基地址0x7C00-1=0xFFFF EFFF,这样最高就只能触及0x7BFF这个物理地址,刚好4KB
;第五步:(实际使用的段界限0xFFFF EFFF-0xFFF)除以0x1000 = 栈段描述符中的界限值0xF FFFE
;!!!!!!!!!!!!!!!!!ESP变化范围限定在0xFFFF F000~0xFFFF FFFF!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
