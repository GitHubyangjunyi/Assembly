...
;但是少用一条指令似乎更好
;既然已经知道新描述符应该安装在哪里,第285~286行,访问ES所指向的4GB内存段,将EDX:EAX中的64位描述符写入由EDI指向的偏移处
...
;栈是向下扩展的,访问32位栈要使用ESP,其最大值是0xFFFF FFFF,因此ESP的值只允许在0xFFFF DFFF和0xFFFF FFFF之间变化,共8KB
