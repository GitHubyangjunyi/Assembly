...
start:                                      ;内核入口点,在执行到这里时,主引导程序已经加载了内核,并对它进行了前期的初始化工作
         ...
         mov edi,salt                       ;C-SALT表的起始位置,这是第一个条目的位置,将其传送到EDI
         mov ecx,salt_items                 ;C-SALT表的条目数量,将其传送到ECX
  .b3:
...
;
;想象以下,应用程序的编写者通过钻研,知道了操作系统数据段的选择子,而且希望用这个选择子访问操作系统的数据段,当然,他不可能在应用程序里访问操作系统数据段
...
;                    |                           |
;                    |                           |
;                    |                           |<---- SS:EBP+8    从SS:EBP+8开始,是pushad指令压入的8个双字
;                     ---------------------------
;                    |      0      |      DS     |<---- SS:EBP+4
;                     ---------------------------
;                    |      0      |      ES     |<---- SS:EBP
...
;我们的用户程序很简单,不会划分为太多的段,16字节的空间可以安装20个描述符,应当足够了,LDT的线性起始地址是登记在TCB内偏移0x0C处的,LDT的界限是登记在TCB内偏移0x0A处的
...
;地854~855行,访问任务的TCB,从用户程序头部内取出栈段选择子和栈指针,以及代码段选择子和入口点,并将它们顺序压入当前的0特权级栈中,这部分内容要结合第13章的用户程序头部来分析
;第864行,执行一个远返回指令retf,假装从调用门返回,于是控制转移到用户程序的3特权级代码开始执行,注意这里用的0特权级栈并非是来自于TSS,不过处理器不会在意这个
;下此,从3特权级的段再来到0特权级执行时,就会用到TSS中的0特权级栈了
;
