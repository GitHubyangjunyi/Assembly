...
allocate_a_4k_page:                         ;分配一个4KB的页
         ...
         mov ds,eax
;第335~341行,从头开始搜索位串,查找空闲页,具体地说就是找到第一个为0的比特,并记下它在整个位串中的位置
         xor eax,eax                        ;清零eax表示从位串的第一个比特开始
  .b1:
         bts [page_bit_map],eax
         jnc .b2                            ;CF标志为0则转移
         inc eax
         cmp eax,page_map_len*8             ;判断是否已经测试了位串中的所有比特,防止越界,page_map_len是位串的字节数,乘以8就是位串的比特数
         jl .b1
  ...
  .b2:;第348行,将该比特在位串中的位置数值乘上页的大小0x1000=4096,就是该比特所对应的页的物理地址
...
alloc_inst_a_page:                          ;分配一个页，并安装在当前活动的
         ...
         mov ds,eax
;EBX=0x8010 0000=0x80?? ????
         ;检查该线性地址所对应的页表是否存在(因为已经知道0x0xFFFF F???这个线性地址访问的是页目录表,下面的几行代码就是将ebx的高10位当成索引,访问页目录表)
         mov esi,ebx                        ;esi=0x8010 0000线性地址的高10位是页目录表的索引,将该值乘以4就是当前页目录表中,与该线性地址对应的页目录项
         and esi,0xffc00000                 ;esi=0x8000 0000保留高10位作为页目录表的索引
         shr esi,20                         ;esi=0x0000 0800得到页目录索引，并乘以4(本来是要移动22位后再乘上4,换成移动20位,效果一样)
         or esi,0xfffff000                  ;页目录自身的线性地址+表内偏移(可以用线性地址来访问页目录表,且当前页目录表的线性地址是0xFFFF F000,因此第373行,用刚才计算出的偏移量和页目录表的线性基地址相加就可以得到要访问的那个目录项的线性地址)
                                            ;至此ESI=0xFFFF F800,相当于访问页目录表偏移800处的内容,0x80?? ????
         test dword [esi],0x00000001        ;P位是否为“1”。检查该线性地址是 
         jnz .b1                            ;否已经有对应的页表,为程序管理器的TSS分配空间时已经有对应的页表了,跳转到.b1
  ...
  .b1:
         ;开始访问该线性地址所对应的页表
         mov esi,ebx                        ;esi=ebx=0x8010 0000
         shr esi,10                         ;esi=0x0020 0400
         and esi,0x003ff000                 ;esi=0x0020 0000;或者0xfffff000，因高10位是零 
         or esi,0xffc00000                  ;esi=0xFFE0 0000得到该页表的线性地址,至此esi=FFE0 0000=1111 1111 11 10 0000 0000 0000 0000 0000,前10位是页目录表索引,后10位就是当前要访问的页表的索引
         
         ;得到该线性地址在页表内的对应条目（页表项） 
         and ebx,0x003ff000                 ;执行后ebx=0x0010 0000
         shr ebx,10                         ;相当于右移12位，再乘以4
         or esi,ebx                         ;页表项的线性地址 
         call allocate_a_4k_page            ;分配一个页，这才是要安装的页
         or eax,0x00000007                  ;页的属性值为0x007,US=1,特权级别3的程序也可以访问,RW=1,可读可写,P=1.页以位于内存中,可以使用
         mov [esi],eax 
...
;------------------------------------------------------------------------------- 
         ...
         ;内核信息
         core_next_laddr  dd  0x80100000    ;内核空间中下一个可分配的线性地址,每当分配了新的内存空间后,该双字将修正位下一个可分配的线性地址
...
start:
         ...
         ;准备打开分页机制,首先必须创建页目录和页表,每个任务都有自己的页目录和页表,内核也不例外
  ...
  .b1:
         ...
         ;在页目录内创建指向页目录自己的目录项
         mov dword [es:ebx+4092],0x00020003 ;第936行,将页目录表的物理地址登记在它自己的最后一个目录项内,页目录最大4KB,最后一个目录项的偏移量是0xFFC,即十进制4092,页目录需要频繁地进行修改,为了方便用线性地址访问页目录表本身,需要使用这项技术
;填写的内容是0x0002 0003,该数值的前20位是物理地址的高20位,P=1,页位于内存中,RW=1,该目录项指向的页表可读可写,US=0,此目录项指向的页表不允许特权级3的程序和任务访问,这将浪费一个页目录表项,同时使得最高端的4MB内存无法访问(0xFFC0 0000~0xFFFF FFFF),即使不浪费,一般的软件也不会涉足这个区域
  ...
  .b3:                                      ;其余的页表项置为无效
         ...
         mov cr3,eax
;第964~966行,先读取CR0的原始内容,再将其最高位置1,接着写回CR0,开启分页机制,从这一瞬间开始,段部件产生的地址不再被看成物理地址,而是要送往页部件进行变换,以得到真正的物理地址
         mov eax,cr0
         or eax,0x80000000
         mov cr0,eax                        ;开启分页机制
;第969~973行,在内核的页目录表中创建一个和线性地址0x8000 0000对应的目录项,并使它指向同一个页表,毕竟我们只改变了线性地址空间范围,内核的数据和代码还是在原来的页内,没有改变
         ;在页目录内创建与线性地址0x80000000对应的目录项
         mov ebx,0xfffff000                 ;实际上给出了当前正在使用的页目录表自己的线性基地址0xFFFF F000
         mov esi,0x80000000                 ;映射的起始地址,第970行,给出了要修改的那个目录项所对应的线性基地址,其高10位乘上4决定了该线性地址所对应的页目录表内偏移量,因此第971行,将线性地址右移22位,只保留高10位,第972行,再将它左移2位,相当于乘上4
         shr esi,22                         ;线性地址的高10位是目录索引,esi逻辑右移22位,esi=0x0000 0200
         shl esi,2                          ;到这esi=0x0000 0800,书上写的0x0000 0200???看上面,因为是倒过来,先从结果着手的,可能有点绕
         mov dword [es:ebx+esi],0x00021003  ;写入目录项（页表的物理地址和属性）[es:ebx+esi]=0xFFFF F800
                                            ;目标单元的线性地址为0xFFFFF200
;第977~979行,先取得GDT的线性基地址,并传送到EBX寄存器,准备开始访问GDT内的段描述符
         ;将GDT中的段描述符映射到线性地址0x80000000
         sgdt [pgdt]                        ;将GDT信息保存到标号pgdt处
         
         mov ebx,[pgdt+2]                   ;前两字节是界限,后4字节是基地址,EBX=GDT的线性基地址
;第981~986行,依次找到内核栈段,文本模式下的视频缓冲区段,公共例程段,内核数据段和内核代码段的描述符,并将每个描述符的最高位改成1
         or dword [es:ebx+0x10+4],0x80000000;在偏移的基础上加上4就是高32位
  ...
  .b4:
         ...
         ;为程序管理器的TSS分配内存空间
         mov ebx,[core_next_laddr]          ;先访问内核数据段,从标号处取得当前可用的线性地址,将来要作为内核TSS的起始线性地址,当前作为参数调用过程alloc_inst_a_page
         call sys_routine_seg_sel:alloc_inst_a_page
         add dword [core_next_laddr],4096
